...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form method="GET" action="drp_perikanan.php"
                    ...
                    <select name="sort" id="sort" class="form-select w-auto">
                        <option value="nama_asc" <?php if (isset($_GET['sort']) && $_GET['sort'] == 'nama_asc')
                            echo 'selected'; ?>>Nama (A-Z)</option>
                        <option value="nama_desc" <?php if (isset($_GET['sort']) && $_GET['sort'] == 'nama_desc')
                            echo 'selected'; ?>>Nama (Z-A)</option>
                        <option value="penempatan" <?php if (isset($_GET['sort']) && $_GET['sort'] == 'penempatan')
                            echo 'selected'; ?>>Penempatan</option>
                    </select>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if (!empty($row["foto_filename"]) || file_exists($foto_path)) {
                    echo '<img src="https://sikep.mahkamahagung.go.id/uploads/foto_pegawai/' . $row['foto_filename'] . '" class="profile-pic">';
                } else {
                    echo '<img src="https://static.vecteezy.com/system/resources/thumbnails/003/337/584/small_2x/default-avatar-photo-placeholder-profile-icon-vector.jpg" alt="Foto tidak tersedia" class="profile-pic">';
                }
                echo '</div>'; // col-lg-3
